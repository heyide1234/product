        ...
        <el-form-item label="序列号">
          ...
          >
            <el-option
              ...
              v-for="(item, key) of SerialNumberList"
              :key="key"
            ></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    sc(row) {
      console.log(row);
      this.lld(row._id); //改变状态
      //生成成品入库单
      let dt = {
        SerialNumber: row.SerialNumber, //序列号
        ProcessName: row.ProcessName, //工艺名称
        ...
        Company: row.Company, //单位
        DeliverGoodsApply: row.Number, //产品入库申请数
        DeliverGoods: "0", //入库数量
    ...
    lld(id) {
      this.$https({
        ...
        data: {
          table: "processList",
          id: id,
          form: { status: "4", creater3: sessionStorage.getItem("loginName") },
        },
      })
        .then(function (res) {
          console.log(res);
    ...
    find() {
      this.$https({
        ...
        params: {
          table: "processList",
          where: { status: "3", creater2: sessionStorage.getItem("loginName") },
        },
      })
        .then((res) => {
          this.tableData = res;
